...
scores = cross_validation.cross_val_score(alg, titanic[predictors], titanic["Survived"], cv=3, scoring='accuracy')
...
def get_family_id(row):
    ...
    #print last_name, row["FamilySize"], family_id
...
# Check the counts of each unique title.
    #    print(ppdd.value_counts(titanic_test["Title"]))
...
# Fit the algorithm using the full training data.
    ...
    print titanic_test[predictors].astype(float)
    #predictions = alg.predict_proba(titanic_test[predictors].iloc[test,:].astype(float))[:,1]
